...
static PciInfoList *qmp_marshal_rpc_PciInfoList(CFDictionaryRef args, Error **errp, void *ctx)
{
    Error *err = NULL;
    Visitor *v;
    ...
    visit_type_PciInfoList(v, "return", &ret, &err);
    error_propagate(errp, err);
...
PciInfoList *qmp_query_pci(Error **errp, void *ctx)

{
    const char *cmdname = "query-pci";
    ...
    ret = qmp_marshal_rpc_PciInfoList(cfargs, &err, ctx);
    CFRelease(cfargs);
