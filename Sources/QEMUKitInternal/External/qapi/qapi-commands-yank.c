...

{
    ...
    visit_type_q_obj_yank_arg(v, "arguments", &argp, &err);
    if (err) {
        goto out;
    }
...
static YankInstanceList *qmp_marshal_rpc_YankInstanceList(CFDictionaryRef args, Error **errp, void *ctx)
{
    Error *err = NULL;
    Visitor *v;
    CFDictionaryRef cfret;
    YankInstanceList *ret = {0};
    ...
    visit_type_YankInstanceList(v, "return", &ret, &err);
    error_propagate(errp, err);
    visit_end_struct(v, NULL);
    visit_free(v);
    CFRelease(cfret);
...

{
    ...
    visit_end_struct(v, NULL);
    visit_complete(v, &cfargs);
    ret = qmp_marshal_rpc_YankInstanceList(cfargs, &err, ctx);
    CFRelease(cfargs);
