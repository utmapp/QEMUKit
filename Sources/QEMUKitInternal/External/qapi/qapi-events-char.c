...
{
    q_obj_VSERPORT_CHANGE_arg *arg;
    Visitor *v;
    v = cf_input_visitor_new(data);
    visit_start_struct(v, "event", NULL, 0, &error_abort);
    visit_type_q_obj_VSERPORT_CHANGE_arg(v, "data", &arg, &error_abort);
    visit_end_struct(v, NULL);
    handler(arg->id, arg->open, ctx);

    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_q_obj_VSERPORT_CHANGE_arg(v, "unused", &arg, NULL);
    visit_free(v);
}
