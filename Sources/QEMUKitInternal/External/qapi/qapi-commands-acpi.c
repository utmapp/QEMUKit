...
static ACPIOSTInfoList *qmp_marshal_rpc_ACPIOSTInfoList(CFDictionaryRef args, Error **errp, void *ctx)
{
    Error *err = NULL;
    Visitor *v;
    CFDictionaryRef cfret;
    ACPIOSTInfoList *ret = {0};
    ...
    visit_type_ACPIOSTInfoList(v, "return", &ret, &err);
    error_propagate(errp, err);
...
ACPIOSTInfoList *qmp_query_acpi_ospm_status(Error **errp, void *ctx)

{
    const char *cmdname = "query-acpi-ospm-status";
    ...
    ret = qmp_marshal_rpc_ACPIOSTInfoList(cfargs, &err, ctx);
    CFRelease(cfargs);
