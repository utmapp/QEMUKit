...
static QCryptodevInfoList *qmp_marshal_rpc_QCryptodevInfoList(CFDictionaryRef args, Error **errp, void *ctx)
{
    Error *err = NULL;
    Visitor *v;
    CFDictionaryRef cfret;
    QCryptodevInfoList *ret = {0};
    ...
    visit_type_QCryptodevInfoList(v, "return", &ret, &err);
    error_propagate(errp, err);
...
QCryptodevInfoList *qmp_query_cryptodev(Error **errp, void *ctx)

{
    const char *cmdname = "query-cryptodev";
    ...
    ret = qmp_marshal_rpc_QCryptodevInfoList(cfargs, &err, ctx);
    CFRelease(cfargs);
